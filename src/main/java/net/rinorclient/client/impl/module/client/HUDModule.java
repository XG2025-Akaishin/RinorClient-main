...
import net.rinorclient.client.api.config.setting.EnumConfig;
...
import net.rinorclient.client.util.StreamUtils;
import net.rinorclient.client.util.render.ColorUtil;
...
public class HUDModule extends ToggleModule {
    ...
    private void arrayListRenderModule(Render2DEvent event, ToggleModule toggleModule) {
    ...
    @EventListener
    public void onRenderOverlayPost(Render2DEvent event) {
        if (mc.player != null && mc.world != null) {
            ...
            if (watermarkConfig.getValue()) {
                RenderManager.renderText(event.getContext(), String.format("%s %s (%s%s)",
                        RinorMod.MOD_NAME, RinorMod.MOD_VER,
                        RinorMod.MOD_BUILD_NUMBER, ""), 2.0f, topLeft, getHudColor(rainbowOffset));
                // topLeft += 9.0f;
            }
            ...
            if (potionEffectsConfig.getValue()) {
                for (StatusEffectInstance e : mc.player.getStatusEffects()) {
                    final StatusEffect effect = (StatusEffect) e.getEffectType();
