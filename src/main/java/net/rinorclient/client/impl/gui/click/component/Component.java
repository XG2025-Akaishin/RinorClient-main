package net.rinorclient.client.impl.gui.click.component;
...
public abstract class Component implements Drawable, Globals {
    ...
    protected void fillGradient(DrawContext context, double startX,
        ...
        BufferBuilder buffer = Tessellator.getInstance().begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_COLOR);
        fillGradient(context.getMatrices().peek().getPositionMatrix(), buffer, startX, startY, endX, endY, z, colorStart, colorEnd);
        BufferRenderer.drawWithGlobalProgram(buffer.end());
        //tessellator.draw();
        RenderSystem.disableBlend();
    }
    ...
    private void drawTexturedQuad(Matrix4f matrix, int x0, int x1,int y0, int y1, int z, float u0,float u1, float v0, float v1,float red, float green, float blue, float alpha) {
        RenderSystem.setShader(GameRenderer::getPositionTexColorProgram);//getPositionColorTexProgram
        RenderSystem.enableBlend();
        //BufferBuilder buffer = Tessellator.getInstance().getBuffer();
        BufferBuilder buffer = Tessellator.getInstance().begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_COLOR_TEXTURE_LIGHT_NORMAL);//POSITION_COLOR_TEXTURE
        buffer.vertex(matrix, (float) x0, (float) y0, (float) z)
